"""
Reconciles detected findings with agent-memory records (git-markdown agent-brain
backend since agent-memory v0.2.0) and returns a ReportSummary. Phase 1 is
hardcoded to type='reference', scope=['global'], source_agent='skills-auditor'
(spec §7 governance).
"""
...
def _fetch_existing_skills_audit_memories(am) -> dict[str, "am.Memory"]:
    ...
    First-run bootstrap: if the backend has never seen a 'skills-auditor'
    source agent, a filtered read may error rather than return empty (this was
    Notion-select behavior pre-v0.2.0; kept as a harmless guard). Treat as an
    empty existing set — the record is created on the first `write()`.
    """
    try:
        ...
        if SOURCE_AGENT in msg and "not found" in msg:
            logger.info(
                "first run: no prior 'skills-auditor' findings in agent-memory "
...
def report(
    ...
    try:
        ...
        logger.error("agent-memory backend unreachable: %s", e)
        _print_local_fallback(findings)
        sys.exit(2)
...
def _print_local_fallback(findings: list[Finding]) -> None:
    ...
    print("=" * 72)
    print("skills-auditor: agent-memory write unavailable — findings below")
    print("=" * 72)
    for f in findings:
